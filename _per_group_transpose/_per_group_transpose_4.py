...
def _per_group_transpose(
    ...
    # Loop over M-blocks assigned to this program
    for start_m in tl.range(0, num_tokens_of_expert, BLOCK_SIZE_M * num_m_programs):
        ...
        # ---- COALESCED STORE ----
        # Transpose the loaded tile so that the inner dimension of the store
        # becomes M (contiguous) instead of K.
        trans_data = tl.trans(data)
        trans_mask = k_mask[:, None] & m_mask[None, :]
        trans_off = k_coord[:, None] * num_tokens_of_expert + m_coord[None, :]
        tl.store(trans_data_start_ptr + trans_off, trans_data, mask=trans_mask)
...
def per_group_transpose(
    ...
    # Use a fixed number of M-blocks per expert to distribute work
    num_m_blocks = 4  # tune based on cores (24)
    grid = lambda META: (
        num_experts,
        num_m_blocks,
    ...
    _per_group_transpose[grid](
        a, trans_a, expert_offsets, k, M_ALIGNMENT, BLOCK_SIZE_M=64, BLOCK_SIZE_K=64
    )
    return trans_a