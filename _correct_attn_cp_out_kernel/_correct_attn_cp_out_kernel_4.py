...
def _lse_global_kernel(
    ...
    for start in range(0, N_ROUNDED, TILE_N):
        ...
        lse_sum = lse_sum * tl.exp(lse_max - new_max) if lse_max > -float("inf") else 0.0
...
def correct_attn_cp_out(outputs, lses, lse_idx):
    ...
    # Second pass: apply correction with D‑dimension tiling
    TILE_D = 128
    D_tiles = (D + TILE_D - 1) // TILE_D
    grid_correct = (B, H, D_tiles)
    _correct_attn_cp_out_kernel[grid_correct](
        outputs,
        new_output,
        lses,
        vlse,
