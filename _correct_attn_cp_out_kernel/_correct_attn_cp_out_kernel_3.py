...
def _lse_global_kernel(
    ...
    TILE_N: tl.constexpr = 128  # smaller tile reduces register pressure further
    lse_max = -float("inf")
    lse_sum = 0.0
    ...
    for start in range(0, N_ROUNDED, TILE_N):
        ...
        # stable online log-sum-exp without conditional (relies on exp(-inf)=0)
...
def correct_attn_cp_out(outputs, lses, lse_idx):
    ...
    # D‑tiling is done inside the kernel with smaller tile for better occupancy
    _correct_attn_cp_out_kernel[(B, H)](
        ...
        lse_idx,
        HEAD_DIM=D,
        TILE_D=64,        # smaller tile reduces register pressure
        N_ROUNDED=N_ROUNDED,
    )
