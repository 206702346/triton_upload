...
def _silu_mul_fp8_quant_deep_gemm(
    ...
    # Mask for valid columns in this group (handles partial last group)
    mask = (g * GROUP_SIZE + cols < H)
    ...
    for t in tl.range(0, n_tokens, num_stages=NUM_STAGES):
        ...
        y_abs_max = tl.max(tl.abs(y))
        y_s = tl.maximum(y_abs_max, eps) * inv_fp8_max
        if use_ue8m0:
            y_s = tl.exp2(tl.ceil(tl.log2(y_s)))

        inv_y_s = 1.0 / y_s
        y_q = tl.clamp(y * inv_y_s, fp8_min, fp8_max).to(y_q_ptr.dtype.element_ty)
...
def persistent_masked_m_silu_mul_quant(
    ...
    # Use optimal group size (tunable)
    group_size = 256
