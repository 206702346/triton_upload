...
def _compute_block_size_and_warps(dim: int):
    ...
    sqrt_dim = int(math.isqrt(dim))
    candidate = _next_power_of_two(sqrt_dim)
    block_size = max(16, min(128, candidate))
    # Set num_warps based on block size: larger blocks get more warps.
...
def _state_passing_fwd(
    ...
    # Compute adaptive block size and num_warps
