...
def _chunk_state_fwd_kernel(
    ...
    # 2D grid: pid_m from first dim, pid_n and combined batch/chunk/head from second dim
    pid_m = tl.program_id(axis=0).to(tl.int64)

    # second dimension: number of (dstate tiles) * (batch * nchunks * nheads)
    num_pid_n = tl.cdiv(dstate, BLOCK_SIZE_N)
    combined = tl.program_id(axis=1).to(tl.int64)
    pid_n = combined % num_pid_n
    pid_comb = combined // num_pid_n  # index into (batch * nchunks * nheads)

    # extract head, chunk, batch from pid_comb (order: head varies fastest, then batch*chunk)
    pid_h = pid_comb % nheads
    pid_bc = pid_comb // nheads
    pid_c = pid_bc // batch
...
def _chunk_state_fwd(
    ...
    # 2D grid: first dim = number of hdim tiles, second dim = (number of dstate tiles) * (batch * nchunks * nheads)
    BLOCK_SIZE_M = 32
    BLOCK_SIZE_N = 32
    num_pid_m = triton.cdiv(headdim, BLOCK_SIZE_M)
    num_pid_n = triton.cdiv(dstate, BLOCK_SIZE_N)
    total_comb = batch * nchunks * nheads
    grid = (num_pid_m, num_pid_n * total_comb)
